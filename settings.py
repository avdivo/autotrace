...
# Размер области изображения (ширина, высота) для образцов
SAMPLE_AREA_SIZE: Tuple[int, int] = (64, 64)
"""Размер области изображения (ширина, высота) для эмбеддинга и сравнения образцов (по умолчанию: 64x64 пикселей).
Используется для выделения участка изображения вокруг указателя мыши при обработке событий ввода."""
...
SAMPLE_SIMILARITY_THRESHOLD: float = 0.90
...
OPENCV_MATCH_THRESHOLD: float = 0.85
"""Порог совпадения для поиска образцов на скриншоте с использованием OpenCV (по умолчанию: 0.85)."""

# Интервал (в секундах) между снимками экрана в процессе мониторинга
SCREEN_MONITOR_INTERVAL: float = 0.1
"""Интервал (в секундах) между снимками экрана в процессе мониторинга (по умолчанию: 0.1 сек)."""

# Порог различия хэшей для определения изменений экрана
SCREEN_HASH_DIFF_THRESHOLD: float = 0.05
"""Порог различия хэшей для определения значительных изменений экрана (по умолчанию: 0.05)."""
