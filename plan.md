...
### Функции:
- `play` — без параметров выполняет команды из временного файла, с параметрами — из указанного файла (только имя, без расширения). Вызывает метод `play_all` объекта `Player` модуля `player`.
- `play_one` — выполняет одну переданную команду:
  ...
  - `play...` — передается функции `play`.
- `record` — без параметров сохраняет команды во временный файл, с параметром — в указанный `.json` файл (список в формате json).
- `cli_main` — точка входа для запуска из командной строки. Осуществляет:
  ...
  - Вызов соответствующих функций (`play`, `play_one`, `record`) в зависимости от переданных аргументов
...
```bash
python -m my_app play my_script  # запустит cli_main, который создаст и запустит manager
```
